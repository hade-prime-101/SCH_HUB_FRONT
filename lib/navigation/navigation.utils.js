 ...
 */
export function filterNavigationItems(items, userRoles, featureFlags, removeEmptyGroups = true) {
    return items
        .filter(item => {
        ...
        if (item.type === 'group') {
            const filteredGroup = {
                ...item,
                children: filterNavigationItems(item.children, userRoles, featureFlags, removeEmptyGroups),
            };
 ...
 */
export function findActiveItemId(items, currentPath) {
    for (const item of items) {
 ...
 */
export function getGroupsToExpand(items, activeItemId) {
    if (!activeItemId) {
        return new Set();
    }
    ...
    function traverse(item, parentGroupPath) {
        ...
        if (item.type === 'group') {
            ...
            const found = item.children.some(child => traverse(child, [...parentGroupPath, item.id]));
            if (found) {
                return true;
            }
        }
 ...
 */
export function getItemPath(items, targetItemId) {
    /**
