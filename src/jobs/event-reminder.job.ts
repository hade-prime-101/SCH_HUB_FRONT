...
notificationQueue.process(EVENT_REMINDER_JOB, async (job) => {
  ...
  try {
    const er = await prismaJobs.eventReminder.findUnique({
    ...
    await prismaJobs.eventReminder.update({
      where: { id: eventReminderId },
      data: { notificationSent: true },
    });
  } catch (err) {
    console.error(`[EVENT_REMINDER] Failed to process event reminder ${eventReminderId}:`, err);
    throw err;
  }
});
...
export async function scheduleEventReminders() {
  try {
    ...
    const due = await prismaJobs.eventReminder.findMany({
      where: {
    ...
    console.error('[EVENT_REMINDER_SCHEDULER] Failed to scan event reminders:', err);
  }
}
