    ...
    try {
      ...
      if (res?.tokens?.accessToken) {
        ...
        await fetch("/api/auth/set-cookie", {
          method:  "POST",
          headers: { Check, Circle, "Content-Type": "application/json" },
          body:    JSON.stringify({ Check, Circle, accessToken: res.tokens.accessToken }),
        });
      }
        ...
        {currentStep === "details" && (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="mt-3 space-y-1.5">
              {[
                { Check, Circle, key: "length", label: "At least 8 characters", check: formData.password.length >= 8 },
                { Check, Circle, key: "case", label: "Uppercase and lowercase letters", check: /[a-z]/.test(formData.password) && /[A-Z]/.test(formData.password) },
                { Check, Circle, key: "number", label: "At least one number", check: /[0-9]/.test(formData.password) },
                { Check, Circle, key: "special", label: "At least one special character", check: /[^A-Za-z0-9]/.test(formData.password) },
              ].map(({ Check, Circle, key, label, check }) => (
