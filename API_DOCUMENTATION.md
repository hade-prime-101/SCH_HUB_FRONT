...
1. ~~**Accommodation posting — no role guard.**~~ **Fixed.** `authorize('HOUSE_AGENT', 'SCHOOL_ADMIN', 'SUPER_ADMIN')` added to `POST /marketplace/accommodation`.

2. ~~**`assignRole` allows SUPER_ADMIN escalation.**~~ **Fixed.** `SUPER_ADMIN` removed from the `assignRoleSchema` enum — cannot be assigned via this endpoint.

3. ~~**`POST /community/notices` — no section enforcement.**~~ **Fixed.** Dedicated `createNoticePost` handler forces `section: 'NOTICE_BOARD'` before passing to service.

4. ~~**`group:message` socket — `groupId` missing from parsed schema.**~~ **Fixed.** `groupId` is now validated via `sendMessageSchema.extend({ groupId })` instead of unsafely cast from raw data.

5. ~~**`POST /study/materials/bulk` — silent JSON parse crash on malformed `materials` field.**~~ **Fixed.** JSON parse is now wrapped in try/catch returning a clear `400` error. Also added a check that `materials.length === files.length`.

6. ~~**`requiresTicket` hardcoded to `false` in `createEventSchema`.**~~ **Fixed.** `requiresTicket: z.boolean().default(false)` is now an explicit field in the schema — callers can pass `true` to create ticketed events.

7. ~~**`moduleStubRoutes` imported but never mounted.**~~ **Fixed.** Dead import removed from `src/routes.ts`.

8. ~~**Pagination inconsistency — `searchUsers`/`listUsers` missing `totalPages`/`hasMore` in meta.**~~ **Fixed.** Both controllers now return the full pagination meta shape matching `sendPaginated`.

9. **AI features require `GROQ_API_KEY`.** If not configured, `POST /ai/summarize`, `POST /study/quizzes/generate`, and all `/ai/personal-study` AI endpoints will fail at runtime. The failure mode depends on the Groq client — expect a `500`. No code-level fallback exists.

10. **Pre-existing TS errors in `src/config/openapi.ts` and `swagger-ui-express`.** Type declarations for `@asteasolutions/zod-to-openapi` and `swagger-ui-express` are missing from `node_modules`. These only affect the non-production Swagger UI (`GET /api/docs`) and do not impact runtime behaviour. Install missing `@types` packages or the packages themselves to resolve.
