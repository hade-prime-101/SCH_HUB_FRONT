  ...
  return (
    <div className="p-6">
      <div className="flex justify-between mb-4">
        ...
        <Link href="/study/quizzes/create" className="bg-primary text-primary-foreground px-4 py-2 rounded">
          Create Quiz
        </Link>
      </div>
      <ul className="space-y-3">
        {quizzes.map(q => (
          <li key={q.id} className="bg-card shadow p-4 rounded flex justify-between">
            <div>
              <Link href={`/study/quizzes/${q.id}`} className="font-medium text-primary">
                {q.title}
              </Link>
              <p className="text-sm text-muted-foreground">{q.isDraft ? "Draft" : "Published"} · {q.questions.length} questions</p>
            </div>
            <Link href={`/study/quizzes/${q.id}/attempt`} className="bg-primary/10 text-primary px-3 py-1 rounded">
              Start
            </Link>
          </li>
        ))}
