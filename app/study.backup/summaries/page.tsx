  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/study/summaries/request" className="bg-primary text-primary-foreground px-4 py-2 rounded">
          Request Summary
        </Link>
      </div>
      {summaries.length === 0 ? (
        ...
        <div className="grid gap-4">
          {summaries.map((s) => (
            <div key={s.materialId} className="bg-card shadow rounded p-4 flex justify-between items-center">
              <div>
                <Link href={`/study/summaries/${s.materialId}`} className="font-medium text-primary hover:underline">
                  {s.title}
                </Link>
