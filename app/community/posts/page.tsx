  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/community/posts/new" className="bg-primary text-primary-foreground px-4 py-2 rounded">
          New Post
        </Link>
      </div>
      {posts.map((post) => (
        <div key={post.id} className="bg-card shadow rounded p-4 mb-3 flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2">
              ...
              <Link href={`/community/posts/${post.id}`} className="font-medium text-primary hover:underline">
                {post.title}
              </Link>
            </div>
