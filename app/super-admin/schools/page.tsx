  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <button onClick={() => { setShowForm(true); setEditingId(null); setForm({ name: "", domain: "" }); }} className="bg-blue-600 text-white px-4 py-2 rounded">Add School</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {schools.map(school => (
          <div key={school.id} className="bg-white shadow rounded p-4">
            <p className="font-medium">{school.name}</p>
            <p className="text-sm text-gray-500">{school.domain}</p>
            <div className="mt-2 flex gap-2">
              <button onClick={() => handleEdit(school)} className="text-blue-600">Edit</button>
