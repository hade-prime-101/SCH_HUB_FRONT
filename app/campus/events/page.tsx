  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <Link href="/campus/events/new" className="bg-primary text-primary-foreground px-4 py-2 rounded">Create Event</Link>
      </div>
      <div className="grid gap-4">
        {events.map(ev => (
          <div key={ev.id} className="bg-card shadow rounded p-4 flex justify-between items-center">
            <div>
              <Link href={`/campus/events/${ev.id}`} className="font-medium text-primary hover:underline">{ev.title}</Link>
