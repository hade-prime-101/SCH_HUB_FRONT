  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <button onClick={handleRevokeAll} className="bg-red-600 text-white px-4 py-2 rounded">
          Revoke All
        </button>
      </div>
      {sessions.length === 0 ? <p>No active sessions.</p> : (
        <ul className="space-y-2">
          {sessions.map(s => (
            <li key={s.id} className="bg-white shadow rounded p-3 flex justify-between items-center">
              <div>
                <p className="font-medium">{s.deviceInfo || 'Unknown device'}</p>
                <p className="text-xs text-gray-500">Last active: {s.lastActiveAt ? new Date(s.lastActiveAt).toLocaleString() : 'N/A'}</p>
              </div>
