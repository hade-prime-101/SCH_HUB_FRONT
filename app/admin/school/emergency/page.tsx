  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <button onClick={() => { setShowForm(true); setEditingId(null); setForm({ name: "", phone: "", role: "" }); }} className="bg-primary text-primary-foreground px-4 py-2 rounded">Add Contact</button>
      </div>
      ...
      <div className="space-y-2">
        {contacts.map(c => (
          <div key={c.id} className="bg-card shadow rounded p-3 flex justify-between items-center">
            <div>
              <p className="font-medium">{c.name} ({c.role})</p>
              <p className="text-primary">{c.phone}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => handleEdit(c)} className="text-primary">Edit</button>
