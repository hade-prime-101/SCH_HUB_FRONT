      ...
      {listings.map(item => (
        <div key={item.id} className="bg-card shadow rounded p-4 mb-3 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleModerate(item.id, 'APPROVE')} className="bg-success text-primary-foreground px-3 py-1 rounded text-sm">Approve</button>
            <button onClick={() => handleModerate(item.id, 'REJECT')} className="bg-destructive text-primary-foreground px-3 py-1 rounded text-sm">Reject</button>
          </div>
        </div>
      ))}
    </div>
