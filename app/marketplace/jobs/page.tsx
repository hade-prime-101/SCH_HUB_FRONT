  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <Link href="/marketplace/jobs/new" className="bg-primary text-primary-foreground px-4 py-2 rounded">
          Post a Job
        </Link>
      </div>
      <div className="space-y-3">
        {jobs.map((job) => (
          <div key={job.id} className="bg-card shadow rounded p-4 flex justify-between items-center">
            <div>
              <Link href={`/marketplace/jobs/${job.id}`} className="font-medium text-primary">
                {job.title}
              </Link>
