  ...
  return (
    <div>
      <div className="flex justify-between mb-4">
        ...
        <Link href="/marketplace/accommodation/new" className="bg-primary text-primary-foreground px-4 py-2 rounded">
          List Property (Agent)
        </Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((item) => (
          <div key={item.id} className="bg-card shadow rounded p-4">
            <Link href={`/marketplace/accommodation/${item.id}`} className="font-medium text-primary">
              {item.title}
            </Link>
